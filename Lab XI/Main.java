...
class Point_N extends Element {
    ...
    double norm() {
        try {
            ...
            // divide by zero ( tho there is no need of division)
            e.printStackTrace();
            return 0.0;
        }
    }
...
public class Main {
    ...
    // single thread merge sort - normal with out of array exception hadnles
    ...
    static void merge(Element[] arr, Element[] left, Element[] right) {
        int i = 0, j = 0, k = 0;
        // manual optimsation 1 : not computing length repeatedly but only once
        int leftLength = left.length;
        int rightLength = right.length;

        while (i < leftLength && j < rightLength) {
            // manual optimsation 2 - computing norm only once
            double normLeft = left[i].norm();
            double normRight = right[j].norm();
    ...
    // multi thread merge sort - with manual optimizationss
